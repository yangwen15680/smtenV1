...
- (void)validateLayout
{
	...
	if (self.titleLabel) {
        ...
        if (self.indicator) {
            ...
            int width = 37.0f + labelSize.width;
            self.indicator.frame = CGRectMake((self.containerView.bounds.size.width -width)/2, y-5,37.0f ,37.0f);
            
       

            self.titleLabel.frame = CGRectMake((self.containerView.bounds.size.width -width)/2+37.0f, y, labelSize.width, height);
            y += height;
